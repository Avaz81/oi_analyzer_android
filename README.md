...
## Алерты (Termux-уведомления + Telegram)
Скрипт сам присылает уведомление, когда происходит значимое событие:
- вердикт ✅ ЗАХОДИТЬ (STRONG/MODERATE)
- OI-стена начинает таять
- резкий скачок ATM IV (≥15% между снапшотами)
- пробитие ключевого уровня (VWAP / POC / VAH / VAL)

### 1. Termux-уведомления (работает сразу после установки термукс-api)
```
pkg install termux-api
```
Также поставь приложение **Termux:API** из F-Droid (отдельное приложение,
без него системные уведомления не отправятся).

### 2. Telegram-бот (опционально, дополнительно к Termux)
1. В Telegram напиши **@BotFather** → `/newbot` → следуй инструкциям → получишь **token**.
2. Напиши что угодно своему новому боту (иначе он не сможет писать тебе первым).
3. Узнай свой **chat_id**: открой в браузере
   `https://api.telegram.org/bot<TOKEN>/getUpdates` (подставь свой token) после того,
   как написал боту — в ответе будет `"chat":{"id": ЧИСЛО ...}`.
4. Скопируй `secrets.example.json` в `secrets.json` и впиши token и chat_id:
```
cp secrets.example.json secrets.json
```
   Затем отредактируй `secrets.json` (`nano secrets.json`).

⚠ **Важно:** `secrets.json` содержит секретные данные — НЕ загружай его на GitHub
(репозиторий у тебя публичный). Файл создаётся и остаётся только локально на телефоне.
Если Telegram не настроен — Termux-уведомления всё равно работают, ошибок не будет,
просто выведется информационное сообщение в консоли.
